...
export default function Profile() {
  ...
  const { Form: EditProfileForm, ...editProfileFetcher } = useFetcher();
  const savingProfile = editProfileFetcher.state === 'submitting';

  const { Form: ChangePasswordForm, ...changePasswordFetcher } = useFetcher();
    ...
    <Layout className="tw:flex tw:max-lg:flex-col tw:gap-4">
      ...
      <SimpleCard title="Change password" className="tw:flex-1">
        <ChangePasswordForm method="post" className="tw:flex tw:flex-col tw:gap-y-4" ref={changePasswordFormRef}>
          ...
          <LabelledInput label="New password" type="password" required name="newPassword" />
          <LabelledInput label="Repeat password" type="password" required name="repeatPassword" />
          <div className="tw:flex tw:justify-end">
